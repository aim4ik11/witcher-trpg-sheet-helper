...
function threatBadgeClass(threat: string): string {
  ...
  if (t.includes('deadly') || t.includes('hard') || t.includes('difficult')) return 'badge badge--danger';
  if (t.includes('easy') || t.includes('simple') || t.includes('medium')) return 'badge badge--gold';
  return 'badge badge--muted';
}

function urlLabel(url: string, index: number): string {
  if (url.includes('192.168.') || url.includes('10.0.') || url.includes('172.')) return index === 0 ? 'Wi-Fi' : 'Network';
  return 'URL';
}
    ...
    <div className="dm-dashboard">
      <header className="page-topbar">
        <div className="page-topbar__inner">
          <button type="button" className="ghost" onClick={() => navigate('/local')}>← Local Session</button>
      ...
      <div className="dm-dashboard__body">
        <section className="dm-section">
          <div className="dm-section-head">
            ...
            <div className="dm-section-actions">
              <button type="button" className="btn-sm" onClick={() => setQrOpen(true)}>QR Code</button>
            </div>
          </div>
          {playUrls.length > 0 ? (
            <div className="host-panel">
              {playUrls.map((url, i) => (
                <div key={url} className="host-url-row">
                  ...
                  <code>{url}</code>
                  <button type="button" className="btn-sm" onClick={() => navigator.clipboard.writeText(url)}>Copy</button>
                </div>
              ))}
            </div>
          ) : (
        ...
        <section className="dm-section">
          <div className="dm-section-head">
            ...
            <div className="dm-section-actions">
              <button type="button" className="btn-sm" onClick={() => setInviteOpen(true)} disabled={!server}>+ Add invite</button>
            </div>
          </div>
          <p className="dm-section-hint">Players log in from a browser with their nickname and 4-digit code.</p>
        ...
        <section className="dm-section">
          <div className="dm-section-head">
            ...
            <div className="dm-section-actions">
              <button type="button" className="primary btn-sm" onClick={() => setCreateType('player')}>+ New Player</button>
            </div>
          </div>
          {loading ? (
            <p className="dm-section-hint">Loading...</p>
          ) : players.length === 0 ? (
            <p className="dm-section-hint">No character sheets yet. Creating one also adds a login invite if needed.</p>
          ) : (
        ...
        <section className="dm-section">
          <div className="dm-section-head">
            ...
            <div className="dm-section-actions">
              <button type="button" className="btn-sm" onClick={() => setCreateType('enemy')}>+ New Enemy</button>
            </div>
          </div>
          {enemies.length === 0 ? (
...
function CharCard({
  ...
  const threatBadge = character.type === 'enemy' && (
    profile?.threat
      ? <span className={threatBadgeClass(profile.threat)}>{profile.threat.split('/')[0]?.trim() ?? profile.threat}</span>
      : !isMonster
    ...
    <div className="char-card">
      <div className="char-card-top">
        <div className="char-card-info">
          <button type="button" className="char-name-btn" onClick={onOpen}>{character.name}</button>
      ...
      {character.type === 'player' && (
        <div className="char-login-meta">
          <span>nick <span className="mono">{character.nickname || '—'}</span></span>
          {loginCode && <span>code <span className="code">{loginCode}</span></span>}
        </div>
      )}
      <div className="char-card-actions">
        <button type="button" className="btn-sm" onClick={onOpen}>Open sheet</button>
        <button type="button" className="danger btn-sm" onClick={onDelete}>Delete</button>
      </div>
    </div>
  );
}
