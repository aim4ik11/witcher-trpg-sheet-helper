...
type PendingRequest = { resolve: (data: unknown) => void; reject: (err: Error) => void; timer: NodeJS.Timeout };
...
function startServer(config: SessionConfig): Promise<ServerInfo> {
  return stopServer().then(() => new Promise<ServerInfo>((resolve, reject) => {
    let resolved = false;
    ...
    const startTimer = setTimeout(() => fail(new Error('Server failed to start (timeout)')), 15_000);
...
function requestServer<T>(msg: Extract<HostToServer, { requestId: string }>): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    ...
    pending.set(msg.requestId, { resolve: (data) => resolve(data as T), reject, timer });
...
ipcMain.handle('session:pickConfig', async (): Promise<SessionConfig | null> => {
  ...
  const config = readSessionFromFile(r.filePaths[0])
    ?? normalizeSessionConfig(JSON.parse(fs.readFileSync(r.filePaths[0], 'utf-8')) as Partial<SessionConfig>);
...
ipcMain.handle('credentials:getAll', () =>
  requestServer<PlayerCredential[]>({ type: 'credentials:getAll', requestId: randomUUID() }),
);
ipcMain.handle('credentials:add', (_e, nickname: string, code?: string) =>
  requestServer<PlayerCredential>({ type: 'credentials:add', requestId: randomUUID(), nickname, code }),
);
