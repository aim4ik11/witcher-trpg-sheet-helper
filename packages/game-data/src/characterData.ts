...
export const PHYSICAL_TABLE: Record<number, { hp: number; sta: number; rec: number; stun: number }> = {
...
export const HAND_TO_HAND_TABLE: Record<number, { meleeBonus: number; punch: string; kick: string }> = {
...
export function getSpd(character: { attributes?: Record<string, number>; speed?: number }): number {
  return character.attributes?.spd ?? character.speed ?? 0;
}

export function getLuckStat(character: { attributes?: Record<string, number>; luck?: { max?: number } }): number {
